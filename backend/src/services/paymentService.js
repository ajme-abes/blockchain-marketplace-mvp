...
class PaymentService {
        ...
        try {
          ...
          if (existingBlockchain?.blockchainRecorded) {
            ...
            if (!canRecord) {
              ...
              // ✅ FIXED: Use retry logic for blockchain recording
              console.log('🔄 Using retry logic for blockchain recording...');
              blockchainResult = await blockchainService.recordOrderTransactionWithRetry(orderId, {
          ...
          if (blockchainResult.success && !blockchainResult.alreadyRecorded) {
            // ✅ NEW: Use transaction to update Order and create BlockchainRecord
            await prisma.$transaction(async (tx) => {
              ...
              // ✅ NEW: Create BlockchainRecord entry
              const existingRecord = await tx.blockchainRecord.findUnique({
                where: { txHash: blockchainResult.blockchainTxHash }
              });
              ...
              if (!existingRecord) {
                await tx.blockchainRecord.create({
                  data: {
                    ...
                    blockNumber: blockchainResult.blockNumber?.toString() || null,
                    timestamp: new Date(),
                    status: blockchainResult.isMock ? 'mock' : 'confirmed'
                  }
                });
                console.log('✅ Created BlockchainRecord entry in payment service');
              }
            });
          ...
          // ✅ NEW: Schedule producer payouts
          console.log('💰 Scheduling producer payouts...');
          try {
            const payoutService = require('./payoutService');
            const payoutResult = await payoutService.scheduleProducerPayouts(orderId);

            if (payoutResult.success) {
              console.log(`✅ Payouts scheduled for ${payoutResult.payouts?.length || 0} producers`);
              if (payoutResult.nextPayoutDate) {
                console.log(`📅 Next payout date: ${payoutResult.nextPayoutDate}`);
              }
            } else {
              console.log('⚠️ Failed to schedule payouts:', payoutResult.error);
            }
          } catch (payoutError) {
            console.error('⚠️ Error scheduling payouts:', payoutError);
            // Don't fail payment if payout scheduling fails
          }
  ...
  async verifyWebhookSignature(signature, payload) {
    // ✅ FIXED: Implement proper webhook signature verification
    if (!this.webhookSecret) {
      console.warn('⚠️ CHAPA_WEBHOOK_SECRET not configured, skipping signature verification');
