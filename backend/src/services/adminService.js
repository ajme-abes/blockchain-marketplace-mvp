...
class AdminService {
  ...
  getPreviousPeriodRange(range) {
    ...
    switch (range) {
      ...
      default:
        ...
        return {
          start: new Date(yesterdayDefault.setHours(0, 0, 0, 0)),
          end: new Date(yesterdayDefault.setHours(23, 59, 59, 999))
        };
    }
  }
  ...
  async getEnhancedOverview(range = 'today') {
    try {
      ...
      const previousPeriodRange = this.getPreviousPeriodRange(range);
      ...
      const revenueGrowth = (previousRevenue._sum.totalAmount || 0) > 0 ?
        Math.round((((totalRevenue._sum.totalAmount || 0) - (previousRevenue._sum.totalAmount || 0)) / (previousRevenue._sum.totalAmount || 0)) * 100) : 0;

      const orderGrowth = previousOrders > 0 ?
        Math.round(((ordersInRange - previousOrders) / previousOrders) * 100) : 0;

      const productGrowth = previousProducts > 0 ?
        Math.round(((activeProducts - previousProducts) / previousProducts) * 100) : 0;

      const disputeChange = previousDisputes > 0 ?
        openDisputes - previousDisputes : 0;

      const paymentChange = previousPayments > 0 ?
        pendingPayments - previousPayments : 0;
  ...
  async getEnhancedSystemAlerts() {
    try {
      ...
      // Check for pending producer verifications
      const pendingVerifications = await prisma.producer.count({
        where: { verificationStatus: 'PENDING' }
      });
      ...
      if (pendingVerifications > 0) {
        alerts.push({
          ...
          title: 'Pending Producer Verifications',
          description: `${pendingVerifications} producer(s) waiting for verification approval`,
          severity: pendingVerifications > 10 ? 'HIGH' : 'MEDIUM',
          timestamp: new Date().toISOString()
        });
      }
      ...
      if (openDisputes > 0) {
        alerts.push({
          id: `open-disputes-${Date.now()}`,
          title: 'Open Disputes Requiring Attention',
          description: `${openDisputes} active dispute(s) need resolution`,
          severity: openDisputes > 5 ? 'CRITICAL' : 'HIGH',
          timestamp: new Date().toISOString()
        });
      }
      ...
      if (pendingPayments > 0) {
        alerts.push({
          ...
          description: `${pendingPayments} payment(s) pending for more than 24 hours`,
          severity: 'MEDIUM',
          timestamp: new Date().toISOString()
        });
      }
      ...
      if (lowStockProducts > 0) {
        alerts.push({
          ...
          description: `${lowStockProducts} product(s) have low inventory (≤5 units)`,
          severity: 'LOW',
          timestamp: new Date().toISOString()
        });
      }
      ...
      if (suspendedUsers > 0) {
        alerts.push({
          ...
          description: `${suspendedUsers} user account(s) currently suspended`,
          severity: 'LOW',
          timestamp: new Date().toISOString()
        });
      }
      ...
      if (!blockchainService.isConnected) {
        alerts.push({
          id: `blockchain-disconnected-${Date.now()}`,
          title: 'Blockchain Connection Issue',
          description: 'Blockchain service is not connected. Transactions may not be recorded.',
          severity: 'CRITICAL',
          timestamp: new Date().toISOString()
        });
      }
  ...
  async getPlatformInsights() {
    try {
      ...
      // Calculate growth percentages
      const revenueGrowth = lastMonthRevenue._sum.totalAmount > 0
        ? ((currentMonthRevenue._sum.totalAmount - lastMonthRevenue._sum.totalAmount) / lastMonthRevenue._sum.totalAmount * 100)
        : 0;

      const orderGrowth = lastMonthRevenue._count.id > 0
        ? ((currentMonthRevenue._count.id - lastMonthRevenue._count.id) / lastMonthRevenue._count.id * 100)
        : 0;

      // Average order value
      const averageOrderValue = currentMonthRevenue._count.id > 0
        ? currentMonthRevenue._sum.totalAmount / currentMonthRevenue._count.id
        : 0;
      ...
      // Payment success rate
      const [totalOrders, successfulPayments] = await Promise.all([
        prisma.order.count({
          where: { orderDate: { gte: lastMonth } }
        }),
        prisma.order.count({
          where: {
            orderDate: { gte: lastMonth },
      ...
      const paymentSuccessRate = totalOrders > 0
        ? Math.round((successfulPayments / totalOrders) * 100)
        : 0;
