...
const ProductDetail = () => {
  ...
  // Main content component (used for both authenticated and non-authenticated)
  ...
  // Return with sidebar for authenticated users, regular layout for guests
  if (isAuthenticated) {
    return (
      <SidebarProvider>
        <div className="min-h-screen flex w-full">
          ...
          <div className="flex-1 flex flex-col">
            <PageHeader
              title={product.name}
              description={`${product.category} • ${product.region}`}
            />
